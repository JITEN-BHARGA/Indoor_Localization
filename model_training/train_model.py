...
def build_sequence_reference(df: pd.DataFrame):
    reference = []
    grouped = df.groupby(['Location', 'Sequence Number'])
    for (location, sequence_number), group in grouped:
        signals = group.groupby('MAC Address')['RSSI value'].mean().to_dict()
        reference.append({
...
def main():
    ...
    reference = build_sequence_reference(df)
    with open(output_dir / 'reference_fingerprints.json', 'w', encoding='utf-8') as f:
    ...
    print(f'Reference sequences:  {len(reference)}')
