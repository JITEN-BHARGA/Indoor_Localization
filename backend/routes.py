...
from backend.mqtt_consumer import is_mqtt_ready, publish_scan_command
from backend.schemas import CreateEspDeviceBody, IngestPayload
...
def trigger_scan(body: ScanRequestBody):
    ...
    object_id = (body.object_id or body.device_id).strip()
    ...
    request_id = create_scan_request(device_id=device_id, object_id=object_id)
    publish_scan_command(device_id=device_id, request_id=request_id, object_id=object_id)
...
@router.post('/predict')
def predict_scan(payload: IngestPayload):
    return hybrid_predict(payload.model_dump())
